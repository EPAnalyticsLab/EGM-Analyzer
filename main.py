...
# ──────────────────────────────────────────────────────────────────────────────
# Colormaps. The default is perceptually uniform and colorblind-safe.
#   - "Viridis" for sequential metrics (LAT, Vpp, ROR) — default
#   - "RdBu_r"  for divergent bipolar traces (zero-centred)
# Users may select alternative colormaps (including Jet, Rainbow, Turbo)
# via the Colormap dropdown in the toolbar. The default remains Viridis.
# ──────────────────────────────────────────────────────────────────────────────
...
app.layout = dbc.Container(fluid=True,
    ...
    dbc.Row([
        ...
        dbc.Col([
            dbc.Button("💾 Save Session (.pkl)",id="btn-save-session",n_clicks=0,
...
html.Div(
    ...
    children=[
        ...
        children=[
            ...
            # R1.16 — Colormap selector with visual colour-bar swatches
            lspan("Colormap:"),
            dcc.Dropdown(
                id="cmap-select",
                options=[
                    ...
                    for c in [
                        ...
                        {"name":"Plasma",
                         "css":"linear-gradient(to right,#0d0887,#7e03a8,#cc4778,#f89540,#f0f921)"},
                        {"name":"Magma",
                         "css":"linear-gradient(to right,#000004,#3b0f70,#8c2981,#de4968,#fde2a3)"},
                        {"name":"Inferno",
                         "css":"linear-gradient(to right,#000004,#420a68,#932667,#dd513a,#fca50a,#fcffa4)"},
                        {"name":"Cividis",
                         "css":"linear-gradient(to right,#00204d,#31446b,#666870,#9b9b6d,#d1cf5b,#ffea46)"},
                        # ── diverging ──
                        {"name":"RdBu",
                         "css":"linear-gradient(to right,#b2182b,#f4a582,#f7f7f7,#92c5de,#2166ac)"},
                        # ── legacy (not colorblind-safe) ──
                        {"name":"Jet",
                         "css":"linear-gradient(to right,#00007f,#0000ff,#007fff,#00ffff,#7fff7f,#ffff00,#ff7f00,#ff0000,#7f0000)"},
                        {"name":"Rainbow",
                         "css":"linear-gradient(to right,#7f00ff,#0000ff,#00ffff,#00ff00,#ffff00,#ff7f00,#ff0000)"},
                        {"name":"Turbo",
                         "css":"linear-gradient(to right,#23171b,#4076f6,#26d0cd,#75f046,#f0a012,#e6460e,#900c00)"},
                    ]
                ],
                value="Viridis",
                clearable=False,
...
def save_session(n_clicks):
    if not n_clicks:
        ...
        return no_update
    if SIGNALS is None and GEOMETRY is None:
        from dash import no_update
        return no_update
    try:
        import io as _io, pickle as _pkl
        payload = {"signals": SIGNALS, "geometry": GEOMETRY}
        buf = _io.BytesIO()
        _pkl.dump(payload, buf)
        buf.seek(0)
        return dcc.send_bytes(buf.getvalue(), "egm_analyzer_session.pkl")
    except Exception as e:
        import traceback; traceback.print_exc()
        from dash import no_update
        return no_update
...
def load_ensitex_from_path(n_clicks, wave_path, lat_path):
    global SIGNALS
    ...
    try:
        SIGNALS = parse_ensitex_from_paths(wave_path, lat_path)
        dt     = SIGNALS["data_table"]
        groups = dt["pt number"].unique()
        opts   = [{"label":str(g),"value":g} for g in groups]
        return opts, False, f"✅ EnsiteX: {len(groups)} freeze groups"
    except Exception as e:
        import traceback; traceback.print_exc()
        return [], True, f"❌ {e}"
...
def render_geo(map_type, cbar_range, group_id, interp_toggle, sigma, omni_type, scatter_opacity, interval, cmap_choice):
    ...
    try:
        ...
        # Use the user-selected colormap for sequential parameter maps (default: Viridis)
        _cmap_seq = cmap_choice if cmap_choice else CMAP_SEQUENTIAL
        ...
        if map_type=="lat":
            ...
            title,colorscale="Vpp Bipolar max (mV)",_cmap_seq
            ...
            title=f"Vpp Omnipolar ({omni_type}) (mV)"; colorscale=_cmap_seq
            ...
            title="ROR (residue/omnipolar)"; colorscale=_cmap_seq
        else:
            return build_geometry_figure(verts,faces,extra)
            ...
            for gc in range(GRID_N):
                ...
                if (gr, gc) in pos_map:
                    ...
                    loop_fig.add_annotation(
                        ...
                        axref=f"x{sp_idx+1}" if sp_idx > 0 else "x",
                        ayref=f"y{sp_idx+1}" if sp_idx > 0 else "y",
                        showarrow=True,
                        arrowhead=2,
                        arrowsize=1.5,
                        arrowwidth=3,
